...
cell_meta <- read.table("data/Cell.Metadata.txt",
                        header = TRUE, sep = "\t") %>%
...
at2_counts_s <- at2_m_s
head(at2_counts_s)

# Construct DESeq2 dataset object
at2_dds <- DESeqDataSetFromMatrix(
  countData = at2_counts_s,
  colData = deseq_metadata_sample,
  design = ~age,
  tidy = FALSE # Would be TRUE if rownames
  # were not already gene names and were instead
  # present as the first column in the counts matrix
)
...
at2_results_s <- results(at2_dds, tidy = TRUE)
head(at2_results_s)

summary(at2_results_s)
```
...
# Options
expr <- rld_at2_m_s # rlog expression matrix of Genes x Samples
