...
# # Subset Seurat object (merged) for AT1/AT2 cells
# seurat_AT1AT2 <- subset(
#   seurat.merged,
#   subset = celltype_level3_fullname == "AT1/AT2"
# )
# 
# at1at2_pseudo_counts <- AggregateExpression(
#   object = seurat_AT1AT2,
#   group.by = "orig.ident",
#   assays = "RNA",
#   slot = "counts" 
# )
# at1at2_matrix <- as.matrix(at1at2_pseudo_counts$RNA)
# 
...
# # Subset AT1/AT2 cell barcodes
# at1at2_cells <- cell_meta %>%
#   filter(CellType == "AT1/AT2") %>%
#   pull(Cell.ID) %>% data.frame
# rownames(at1at2_cells) <- at1at2_cells$.
```
...
## Cluster analysis
```{r}
clusters <- read.table("./data/AT2_gene_hclust_clusters_k12.csv",sep=",",header=T)
cluster_6 <- subset(clusters,cluster==6)
cluster6_genes <- cluster_6[,1]
write.table(cluster6_genes,"./data/AT2_cluster6_genes.txt",row.names=F,quote=F,col.names=F)
```